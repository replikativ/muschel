...
(defn- ancestor-of-mount?
  ...
  [mounts p]
  (boolean (some (fn [m] (and (not= m p) (under? m p))) (keys mounts))))
...
(defrecord MountFS [base mounts cwd-atom]
    ...
    (when-let [p (canonicalize @cwd-atom path)]
      (let [[f cp m] (route base mounts p)]
        (cond
          ...
          (or (contains? mounts p) (ancestor-of-mount? mounts p)) p
  ...
  (-open-sink [this path append?]
    (when-let [p (canonicalize @cwd-atom path)]
      ...
      (when-not (or (contains? mounts p) (ancestor-of-mount? mounts p))
        (let [[f cp _] (route base mounts p)]
          (fs/-open-sink f cp append?)))))

  (-mkdir [this path]
    (when-let [p (canonicalize @cwd-atom path)]
      (if (or (contains? mounts p) (ancestor-of-mount? mounts p))
  ...
  (-rename [this from to]
    (let [pf (canonicalize @cwd-atom from)
      ...
      (when (and pf pt
                 (not (contains? mounts pf))
                 (not (ancestor-of-mount? mounts pf))
                 (not (contains? mounts pt))
...
(defn make
  ...
  {:pre [(every? #(and (string? %)
                       ...
                       (not (str/ends-with? % "/")))
                 (keys mounts))]}
  (->MountFS base-fs (into {} mounts) (atom "/")))
