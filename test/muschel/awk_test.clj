(ns muschel.awk-test
  "Tests for muschel.builtins.awk.

   The corpus at `test/muschel/awk_corpus.edn` (ported from goawk's
   interp_test.go) is the main bench. Each entry is `{:src :in :out}`.
   We also keep a handful of hand-picked tests here for the
   *muschel-specific* surface (-v / -F / -f options and the dispatch
   path through posix.clj/awk via the host)."
    ...
    (when corpus
      (let [results (volatile! {:pass 0 :fail [] :err []})]
        ...
        (let [{:keys [pass fail err]} @results
          ...
          ;; Don't fail the build on individual mismatches — print first
          ;; few for diagnosis. The assertion is just "we ran them all
...
;; ============================================================================
;; Integration through posix.clj/awk + the host (-v, -F, -f)
;; ============================================================================
